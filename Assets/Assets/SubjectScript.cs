...
public class SubjectScript : MonoBehaviour
{
    ...
    {  //                D >5 & ! laying --> Move Forward D
        ...
        if (thisAnimator.GetCurrentAnimatorStateInfo(0).IsName("laying") && !is123Called && didHeGoThere)
        {
        ...
        // pump without calling 123
        if (!is123Called && pumpBox.GetComponent<PumpScriptPractice>().countPump > 0 && !mobileMsgShown)
        {
            mobileMsgShown = true;
            StartCoroutine(ShowMessageBad());
        }
        else if (is123Called && pumpBox.GetComponent<PumpScriptPractice>().countPump > 0)
        {
    ...
    IEnumerator ShowMessageBad()
    {
        ...
        MobileText.GetComponentInChildren<TextMesh>().text = "You forgot you called 123!";
        yield return new WaitForSeconds(2);
        MobileText.SetActive(false);
    }
}